...
Die Umschlag-Anweisung verlangt streng gültiges JSON: verdoppelte Backslashes, escapte Anführungszeichen und `\n` statt roher Zeilenumbrüche. Scheitert das strikte Parsen, repariert die Bridge drei strukturell eindeutige Fehler innerhalb von String-Literalen — einen Backslash vor einem Zeichen, das kein JSON-Escape ist (etwa `C:\Users`), rohe Steuerzeichen und ein Anführungszeichen, auf das nach optionalem Leerraum weder `,` noch `}`, `]`, `:` oder das Textende folgt und das daher kein Stringende sein kann (etwa `test -z "$(find …)"` in einem Shell-Befehl, T-954) — sowie außerhalb von Strings ein nicht geschlossenes Objekt vor `]`, bei dem `}` die einzige Fortsetzung ist (etwa `"timeout":90}]}` statt `}}]}`, T-959; offene Klammern am Textende werden nicht ergänzt, weil das eine abgeschnittene Antwort zu einem unvollständigen Aufruf machen würde), und protokolliert die Reparatur. Jeder andere Fehler bleibt fail-closed; die Fehlermeldung nennt dann die Länge des Rohtexts, seine ersten 120 Zeichen und einen Ausschnitt von bis zu 320 Zeichen um die von serde_json gemeldete Fehlerstelle, markiert mit `>>>HIER<<<` (T-952, T-955).

Der Umschlag wird in einem Markdown-Codeblock angefordert und aus dessen `pre`-Element gelesen (`TXENV` in `src/browser/js.rs`). Gerenderter Fliesstext verliert beim Auslesen Zeichen: Sternchenpaare werden Kursivschrift, Dollarpaare KaTeX-Formeln, Backticks und Unterstriche Formatierung. Gemessen am 2026-09-14 kamen bei deepseek und qwen ohne Codeblock 0 von 4 Dollarzeichen und 0 bis 4 von 8 Sternchen in Werkzeugargumenten an, mit Codeblock alle. Der Codeblock wird nur genommen, wenn außerhalb davon höchstens 40 Zeichen UI-Beschriftung stehen, damit ein Beispiel-Umschlag in einer erklärenden Antwort kein Aufruf wird; geschützte Leerzeichen (NBSP, bei qwen) werden zu normalen Leerzeichen. Liefert ein Modell den Umschlag trotzdem als Fliesstext, wird er wie bisher gelesen und kann Zeichen verloren haben (T-956).
