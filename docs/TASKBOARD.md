...
Matrix-Stand 2026-09-08 (Live-Reproof + Selector-Nachzug + ID-Roundtrip `model_switch_roundtrip_v2_2026-09-08.md`): **107/130 passed**. `model_switch`: qwen (`Qwen3.7-Plus->Qwen3.8-Max`) + kimi (`Schnell->K3 Swarm`) via Namens-Knoten-Selektoren pass; **gemini (3.6 Flash->3.5 Flash-Lite) + zai (GLM-5.3->GLM-5.3-Flash) via exakter Auswahl-API (ID-Roundtrip) pass 3/3** — Substring-Mehrdeutigkeit geloest. claude (Trigger Sonnet 5 Hoch = Modell+Aufwand, aktives Modell nicht im Erst-Menue) + perplexity (statischer Button Modell ohne Modellname) bleiben strukturell failed — dokumentierte Audit-Grenze, kein `model_id_attr`. Offen (DoD NICHT done): effort-Zellen (6 not_run + zai unreachable), model/auto (not_run), auto RED, zai-GLM-5.3 Webchat flaky 2026-09-07.
