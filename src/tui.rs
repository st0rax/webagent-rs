...
use crate::worker_pool::{
    candidates_with_profile, PoolControl, PoolState, STATUS_ACTIVE, WorkerPool,
};
...
/// Findet die neuste Nachricht (pending in `inbox/` oder erledigt in
/// `inbox/_read/`) eines Brains und liefert (Dateiname, Body-Vorschau, erledigt?).
fn consider_dir(
...
pub fn run_tui(active: usize, brains: &str, poll_secs: u64, headless: bool) -> i32 {
    ...
    loop {
        ...
        if io::stdin().lock().read_line(&mut line).is_err() {
            ...
            write_control(&control_path, &PoolControl { stop: true, ..Default::default() });
            break;
        }
        let input = line.trim();
        ...
        match cmd {
            ...
            "send" => {
                ...
                if brain.is_empty() || text.is_empty() {
                    ...
                    println!("→ '{brain}' ist kein Kandidat (Profile fehlt?). Verfügbar: {:?}", candidates);
                }
            }
            "h" | "help" | "" => {
                println!(
